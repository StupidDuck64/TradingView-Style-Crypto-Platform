...
const SystemHealthCard: React.FC = () => {
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label={t("systemDiagnostics")}
      ...
      {showTooltip && (
        <div className="absolute top-full left-0 mt-2 w-72 p-3 rounded-lg border border-gray-600 bg-gray-900 shadow-2xl z-[220] text-xs">
          ...
          {loading && !healthData && (
            <div className="text-gray-400">{t("loadingHealth")}</div>
          )}
          ...
          {healthData && (
            <div className="space-y-1.5">
              <div className="grid grid-cols-2 gap-1">
                <span className="text-gray-400">{t("apiRtt")}</span>
                <span className="text-gray-200 text-right">
                  {healthData.api_rtt_ms ?? "-"} ms
                </span>

                <span className="text-gray-400">{t("serverHealth")}</span>
                <span className="text-gray-200 text-right">
                  {healthData.status || "-"}
                </span>

                <span className="text-gray-400">{t("totalCheck")}</span>
                <span className="text-gray-200 text-right">
                  {healthData.total_latency_ms ?? "-"} ms
                </span>
              ...
              <div className="border-t border-gray-700 pt-1.5">
                <div className="text-gray-400 mb-1">{t("dependencies")}</div>
                {[
                  { key: "keydb", labelKey: "cache" as const },
                  { key: "influxdb", labelKey: "timeSeriesDb" as const },
                  { key: "trino", labelKey: "queryEngine" as const },
                ].map(({ key: svc, labelKey }) => {
                  const status = healthData.checks?.[svc];
                  const latency =
                    healthData.latency_ms?.[`${svc}_ms`];
                  const ok = status === "ok";
                  return (
                    <div
                      key={svc}
              ...
              <div className="text-[11px] text-gray-500 pt-1 border-t border-gray-700">
                ...
                {healthData.checked_at
                  ? new Date(healthData.checked_at).toLocaleString()
                  : "-"}
              </div>
