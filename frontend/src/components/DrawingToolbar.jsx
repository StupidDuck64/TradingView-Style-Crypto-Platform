...
import ToolSettingsPopup, { DEFAULT_TOOL_SETTINGS } from './ToolSettingsPopup';
import { useI18n } from '../i18n';

// Tools that have a settings popup
...
const TOOL_GROUPS = [
  {
    ...
    tools: [
      ...
      },
      {
        ...
        icon: (
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
            <path d="M3 20L21 4" /><circle cx="3" cy="20" r="2" fill="currentColor" /><circle cx="21" cy="4" r="2" fill="currentColor" />
          </svg>
        ),
      },
      {
      ...
      },
      {
        ...
        icon: (
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
            <path d="M2 4h20" /><path d="M2 9h20" opacity="0.7" /><path d="M2 14h20" opacity="0.5" /><path d="M2 20h20" opacity="0.3" />
          </svg>
        ),
      },
      {
    ...
    tools: [
      {
        ...
        icon: (
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
            <path d="M3 6h18" /><path d="M8 6V4h8v2" /><path d="M5 6l1 14h12l1-14" /><path d="M10 10v8" /><path d="M14 10v8" />
          </svg>
        ),
      },
    ],
...
const DrawingToolbar = ({ activeTool, onToolChange, onClearAll, toolSettings, onToolSettingsChange }) => {
  const { t } = useI18n();
  const [openSettings, setOpenSettings] = useState(null); // tool id whose settings panel is open
  ...
  const handleSettingsClick = useCallback((e, toolId) => {
    e.stopPropagation();
              ...
              >
                <button
                  ...
                  className={`p-2 rounded-md transition-all duration-150
                    ${isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}
                >
                  {tool.icon}
                </button>

                {/* Settings gear — visible on hover or when active */}
                {hasSettings && (
                  <button
                    ...
                    className={`absolute -right-1 -bottom-1 w-4 h-4 rounded-full flex items-center justify-center transition-all
                      ${openSettings === tool.id ? 'bg-blue-400 text-white opacity-100' : 'bg-gray-600 text-gray-300 opacity-0 group-hover:opacity-100'}
                    `}
                    title="Cài đặt"
                  >
                    <Settings size={9} />
                ...
                {openSettings === tool.id && hasSettings && (
                  <ToolSettingsPopup
                    ...
                    onChange={(newSettings) => onToolSettingsChange?.(tool.id, newSettings)}
                    onClose={() => setOpenSettings(null)}
                    anchorRef={{ current: btnRefs.current[tool.id] }}
                  />
                )}
              </div>
            );
