 ...
 *   onClear()                    — called when user clears the selection (back to live)
 *   active                       — whether a historical range is currently applied
 */
const DateRangePicker = ({ onApply, onClear, active = false }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        title={
          active ? "Historical mode — click to change" : "Query historical data"
        }
      >
        {active ? <Clock size={12} /> : <Calendar size={12} />}
        {active ? "Historical" : "History"}
      </button>
